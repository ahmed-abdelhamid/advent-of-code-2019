...
const goToLocation = (startingPoint, wire) => {
  ...
  for (let i of wire) {
    ...
    while (steps > 0) {
      const newLocation = step(direction, { x, y });
      x = newLocation.x;
      y = newLocation.y;
  ...
  for (let i = 0; i < firstWirePositions.length; i++) {
    for (let j = 0; j < secondWirePositions.length; j++) {
      if (
        ...
        crossPoints.push(secondWirePositions[j]);
      }
    }
  }
...
const findManhattenDistance = crossPoints => {
  let distance = Math.abs(crossPoints[0].x) + Math.abs(crossPoints[0].y);

  crossPoints.map(location => {
    if (distance > Math.abs(location.x) + Math.abs(location.y)) {
      distance = Math.abs(location.x) + Math.abs(location.y);
    }
  });
...
console.log(findManhattenDistance(crossPoints));
